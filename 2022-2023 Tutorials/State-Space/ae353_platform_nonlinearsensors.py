...
class Simulator:
    ...
    def get_sensor_measurements(self):
        joint_states = pybullet.getJointStates(self.robot_id, self.joint_ids)
        platform_angle = joint_states[0][0]
        return np.sin(platform_angle) + self.sensor_noise * self.rng.standard_normal()
    ...
    def step(self, controller):
        ...
        # Get the state
        platform_angle, platform_velocity, wheel_angle, wheel_velocity = self.get_state()

        # Get the sensor measurements
        platform_angle_measurement = self.get_sensor_measurements()
        ...
        self.data['platform_angle_measurement'].append(platform_angle_measurement)
        for key in self.variables_to_log:
